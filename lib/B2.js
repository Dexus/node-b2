...
class B2 {
    ...
    confirmAuthorizationToken() {
        return new Promise((resolve, reject) => {
            if (!this.authorizationInfo.authorizationTokenExpireDate) {
                reject('Authorization data is Undefined. Call "authorize_account()" at first.');
                return;
            }
    ...
    */
    ...
    uploadFileWithDir(bucketID, uploadFilePath, uploadDir) {
        return new Promise((resolve, reject) => {
            const uploadFilename = path.join(uploadDir, path.basename(uploadFilePath));
            let uploadFileSize = 0;
            let uploadFileHash = '';
